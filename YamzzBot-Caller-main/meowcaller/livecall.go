...
type Call struct {
	...
	sink         AudioSink
	onReady      func()
	onVideoReady func() // dipanggil segera setelah videoTx pipeline aktif (sebelum peer angkat)
...
// OnVideoReady registers a callback fired as soon as the video send pipeline (videoTx)
// is active — i.e. right after runMedia sets up the video WARP pipeline, BEFORE the peer
// picks up the call. Use this to start SendVideo early so the peer sees video immediately
// when they answer, without waiting for OnReady (which fires only after first inbound RTP).
func (c *Call) OnVideoReady(fn func()) {
	c.mu.Lock()
	c.onVideoReady = fn
	c.mu.Unlock()
}

func (c *Call) onVideoReadyFn() func() {
	c.mu.Lock()
	defer c.mu.Unlock()
	return c.onVideoReady
}
